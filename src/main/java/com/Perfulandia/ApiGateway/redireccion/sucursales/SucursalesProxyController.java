package com.Perfulandia.ApiGateway.redireccion.sucursales;
...
@RestController
@RequestMapping("/api/proxy/sucursales")
@RequiredArgsConstructor
public class SucursalesProxyController {

    private final RestTemplate restTemplate;

    @RequestMapping(value = "/**", method = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE})
    public ResponseEntity<?> proxySucursales(HttpServletRequest request,
                                             @RequestBody(required = false) String body,
                                             @RequestHeader HttpHeaders headers) {

        String originalPath = request.getRequestURI().replace("/api/proxy/sucursales", "");
        String targetUrl = "http://localhost:8003/api/sucursales" + originalPath;
