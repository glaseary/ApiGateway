package com.Perfulandia.ApiGateway.redireccion.ubicaciones;
...
@RestController
@RequestMapping("/api/proxy/ubicaciones")
@RequiredArgsConstructor
public class UbicacionesProxyController {

    private final RestTemplate restTemplate;

    @RequestMapping(value = "/**", method = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE})
    public ResponseEntity<?> proxyUbicaciones(HttpServletRequest request,
                                              @RequestBody(required = false) String body,
                                              @RequestHeader HttpHeaders headers) {

        String originalPath = request.getRequestURI().replace("/api/proxy/ubicaciones", "");
        String targetUrl = "http://localhost:8000/api/ubicaciones" + originalPath;
